...
public:
	...
	inline void setData(double *lat, double *lon, double *alt, 
		double *spd, double *dir,
		CTime *timestamp,
		CString *name);
	...
	inline bool getTimeDate(CTime *timestamp) {
		// this is ugly and should not be an inline!
		// http://www.cplusplus.com/reference/clibrary/ctime/mktime/
		if ((val_time) && (val_date)) {
			INT32 hh,mm,ss;
			INT32 YY,MM,DD;
			...
			CTime t;
			tm tstruct;
			tstruct.tm_year=YY-1900;
			tstruct.tm_mon=MM-1;
			tstruct.tm_mday=DD;
			tstruct.tm_hour=hh;
			tstruct.tm_min=mm;
			tstruct.tm_sec=ss;
			*timestamp=CTime(mktime(&tstruct));
	...
	inline CString formatTime() {
		...
		if (val_time) {
			// @@@ - gps_time is in HHMMSS ... why did I write this as utc type conversion?
			//hh=(int)(gps_time/3600);
			//mm=(int)((gps_time - hh*3600)/60);
			//ss=gps_time - hh*3600 - mm*60;
			hh=(int)(gps_time/10000);
			mm=(int)((gps_time - hh*10000)/100);
			ss=gps_time - hh*10000 - mm*100;
