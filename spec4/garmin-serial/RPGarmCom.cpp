...
, val_lat(0), val_lon(0), val_alt(0), val_spd(0), val_dir(0), val_time(0), val_date(0), val_name(0)
{
	setData(lat,lon,alt,spd,dir,timestamp,name);
}
		...
		CString *name) 
{
	if (timestamp!=NULL) {
		CString tval;
		INT32 txtime,txdate;
		// @@@ totally non-intuitive: cannot modify string lengths!
		//tval=timestamp->FormatGmt("%02H%02M%02S");
		tval=timestamp->FormatGmt("%H%M%S");
		txtime=atol(tval.GetString());
		//tval=timestamp->FormatGmt("%02d%02m%02y");
		tval=timestamp->FormatGmt("%d%m%y");
		txdate=atol(tval.GetString());
		setData(lat,lon,alt,spd,dir,&txtime,&txdate,name);
	}
	else {
		setData(lat,lon,alt,spd,dir,NULL,NULL,name);
	}
