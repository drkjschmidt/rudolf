...
#define CUSTOMER_GPS_TIME 100
		...
		if (theApp.pGPS->updateInfo()) {
			for (ii=0; ii<CUSTOMER_GPS_WAIT*(1000/CUSTOMER_GPS_TIME); ii++) 
			{
				...
				theApp.pGPS->updateInfo(ii==0,0);

				// @@@ we should probably come up with a better validation model ... 
				if (theApp.pGPS->getDeviceName(NULL) && 
					theApp.pGPS->getLoc(NULL,NULL,NULL) &&
					theApp.pGPS->getTimeData(NULL)) {
	...
	{
		theApp.pGPS->updateInfo();
	}

	// @@@ no protection against invalid readings!
	theApp.pGPS->getGPSInfo(&acqGPSInfo);
